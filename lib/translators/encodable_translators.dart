...
class EncodableStringTranslator<T extends Encodable>
    extends Translator<String, T> {
  /// the value that will be used for decodeing the JSON
  final T decoder;

  const EncodableStringTranslator(this.decoder);
...
class EncodableMapListTranslator<T extends Encodable>
    extends Translator<List<Map<String, dynamic>>, List<T>> {
  final T decoder;

  const EncodableMapListTranslator(this.decoder);
...
class EncodableMapTranslator<T extends Encodable>
    extends Translator<Map<String, dynamic>, T> {
  final T decoder;

  const EncodableMapTranslator(this.decoder);

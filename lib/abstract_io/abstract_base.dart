...
abstract class ExternalIO<W,R> extends AbstractIO<W,R>{
  ...
  Future<R> lockAndUpdate(R Function(R newData) update){
    return (ioInterface as ExternalIOInterface).lockAndUpdate<R>(
      update, 
...
abstract class ExternalMapIO<KW,VW, KR,VR> extends ExternalIO<Map<KW,VW>, Map<KR,VR>>{
  ...
  ExternalMapIO(
    ExternalMapIOInterface<KW,VW> ioInterface,
    {
      this.keyTranslator,
  ...
  Future<KR> addEntry(VR value) async {
    return keyTranslator.translateWritable(
      await (ioInterface as ExternalMapIOInterface).addEntry(valueTranslator.translateReadable(value))
    );
  }
  ...
  Future<VR> getEntry(KR key) async {
    return valueTranslator.translateWritable(
      await (ioInterface as ExternalMapIOInterface).getEntry(
        keyTranslator.translateReadable(key)
      )
    );
  }
  ...
  Future<bool> deleteEntry(KR key){
    return (ioInterface as ExternalMapIOInterface).deleteEntry(keyTranslator.translateReadable(key));
  }
...
abstract class Translator<W,R>{
  ...
  dynamic translate(dynamic data){
    if(data is R){
      ...
      throw ErrorSummary("${this.runtimeType} does not translate the type ${data.runtimeType}, supported types are $W and $R");
    }
  }
...
abstract class ExternalIOInterface<W> extends IOInterface<W>{
  ...
  Future<R> lockAndUpdate<R>(R Function(R newData) update, {Translator<W,R> translator});
}
...
abstract class ExternalMapIOInterface<KW,VW> extends ExternalIOInterface<Map<KW,VW>>{

  /// lock and update a specific entry with the given [key] 
  /// 
  /// should follow the same general protocol as [lockAndUpdate]
  Future<V> lockAndUpdateEntry<V>(KW key, V Function(V newData) update, {Translator<VW,V> valueTranslator});
